export default class Arena {
    ...
    placeBomb(x = Math.floor(Math.random()*this.resolution),y = Math.floor(Math.random()*this.resolution)){
            if(this.#grid()[x][y] === '*'){
                this.placeBomb();
            }else{
                this.bombCounter += 1;
            ...
            row.forEach((cell, x)=>{
                ...
                if(cell === ''){
                    ...
                    neighbours.forEach((neighbour)=>{
                        if(neighbour[0] >= 0 && neighbour[1] >= 0 && neighbour[0] <= this.#grid().length-1 && neighbour[1] <= this.#grid()[0].length-1){
